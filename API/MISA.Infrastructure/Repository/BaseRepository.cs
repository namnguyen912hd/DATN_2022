...
namespace MISA.Infrastructure.Repository
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity>, IDisposable where TEntity : BaseEntity
    {
        ...
        public virtual int InsertWithTransaction(BaseEntity entity, IDbTransaction transaction)
        {
            var parameters = MappingDbType(entity);
            var connection = transaction.Connection;
            var rowEffects = connection.Execute($"Proc_Insert{entity.GetType().Name}", parameters, commandType: CommandType.StoredProcedure, transaction: transaction);
            return rowEffects;
        }
        ...
        private DynamicParameters MappingDbType(BaseEntity entity)
        {
            ...
            var properties = entity.GetType().GetProperties().Where(p => !p.IsDefined(typeof(DBNotMap), true)).ToList();
