...
namespace MISA.Infrastructure.Repository
{
    public class ProductRepository : BaseRepository<Product>, IProductRepository
    {
                    ...
                    if (product.EntityState == Core.Enums.MISAEnum.EntityState.AddNew)
                    {
                        ...
                        if (result > 0 && product.Products!=null)
                        {
                            ...
                            foreach (Product productDetail in product.Products)
                            {
                                ...
                                productDetail.ProductImageId = product.ProductImageId;
        ...
        public List<Product> GetProcductsByCategoryID(Guid? categoryID, string productName)
        {
            var parameters = new DynamicParameters();
            parameters.Add($"$CategoryId", categoryID, DbType.String);
            parameters.Add($"$ProductName", productName, DbType.String);
            var lstProduct = _dbConnection.Query<Product>($"Proc_GetProductsByCategoryID", parameters, commandType: CommandType.StoredProcedure).ToList();
            return lstProduct;
        }
    }
}
